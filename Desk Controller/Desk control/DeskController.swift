...
class DeskController: NSObject {
    ...
    func moveToPosition(_ position: Position) {
        beginTargetedMove(to: position)
    }

    func moveToHeight(_ height: Float) {
        beginTargetedMove(to: .custom(height: height))
    }

    /// Go-to-height (F17/F18, sit/stand buttons). The Linak box only moves for
    /// ~1 s per GATT write; `moveIfNeeded` resends using position notifies.
    /// After a night of idle the link can stay `.connected` while notifies
    /// die — one pulse (~2 cm) then stall, and the next F-key is a no-op
    /// (`distSincePrev=0`). Arrow buttons already have `holdTimer`; this path
    /// did not. If notifies look stale, force a reconnect and resume after.
    private func beginTargetedMove(to position: Position) {
        previousPosition = nil
        desk.resubscribePosition()

        if desk.positionNotificationsAreStale {
            dbg("beginTargetedMove: stale notifies (\(String(format: "%.0f", Date().timeIntervalSince(desk.lastPositionNotification)))s) — forceReconnect")
            BluetoothManager.shared.pendingMove = position
